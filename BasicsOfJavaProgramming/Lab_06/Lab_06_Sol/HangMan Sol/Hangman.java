...
public class Hangman implements KeyListener
{
    ...
    private int readWordList(String fileName, String[] wordList) throws IOException 
    {
        ...
        // remember to close the scanner as a good practice
        scanner.close();
    ...
    private String pickSecretWord(String[] wordList, int wordListSize)
    {
        ...
        // Generate a random index in the range of [0, wordListSize - 1]
        int randomIndex = randomNumberGenerator.nextInt(wordListSize);
        
        // Return the word specified by the random index as the secret word
    ...
    private String constructInitialDisplayWord(String secretWord)
    {
        ...
        // Create the secret word by repeated concatenation of the unknown character
        for(int i = 0; i < secretWord.length(); i++) {
            displayWord += UNKNOWN_CHARACTER;
        }
    ...
    private String updateDisplayWord(String displayWord, String secretWord, char guess)
    {
        ...
        // create the new display word by repeated concatenation
        for(int i = 0; i < displayWord.length(); i++)
        {
            // if the character at position i equals the guess, concatenate with the guess character
            if(secretWord.charAt(i) == guess) 
            {
                newDisplayWord += guess;
            }
            
            // otherwise, concatenate with the ith character of the current display word
            else
            {
    ...
    {
        // return if either the secretWord is equal to the displayWord or life equals 0
