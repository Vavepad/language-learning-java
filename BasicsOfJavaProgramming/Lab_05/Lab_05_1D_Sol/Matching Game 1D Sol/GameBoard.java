...
{   
    ...
    {
        Card originalCardA = cards[cardAIndex];
        cards[cardAIndex] = cards[cardBIndex];
        cards[cardBIndex] = originalCardA;
    }
    ...
    {       
        //if any of the cards is flipped facing down, return false immediately
        for(int i=0; i<MatchingGame.NUMBER_OF_CARDS; i++)
        {
            if(!cards[i].isFacingUp()) return false;
        }
        
        //at this point, we know that all cards are flipped facing up, so return true
        return true;
    }
