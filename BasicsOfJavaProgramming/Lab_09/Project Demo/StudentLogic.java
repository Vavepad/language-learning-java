...
public class StudentLogic implements GameLogic {
    ...
    public GameRecord[] updateHighScoreRecords(GameRecord[] highScoreRecords, String name, int level, int score) {
        boolean userExist = this.isUserExist(highScoreRecords, name);
        int minIndex;
        if(userExist) {
            minIndex = highScoreRecords.length;
            ...
            for(int i = 0; i < minIndex; ++i) {
                if(highScoreRecords[i].getName().equals(name)) {
                    ...
                    } else if(highScoreRecords[i].getScore() == score && highScoreRecords[i].getLevel() < level) {
                        highScoreRecords[i].setLevel(level);
                    }
                    break;
                }
            ...
            minIndex = this.minPos(highScoreRecords, highScoreRecords.length);
        ...
        this.sortArray(highScoreRecords, highScoreRecords.length);
    ...
    private void sortArray(GameRecord[] array, int size) {
        ...
        while(remaining > 1) {
            int minIndex = this.minPos(array, remaining);
            --remaining;
            this.swapArrayRows(array, minIndex, remaining);
        }
